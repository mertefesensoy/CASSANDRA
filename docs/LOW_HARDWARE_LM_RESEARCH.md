...
- **Function Vector Shift and Differential Circuit Vulnerability**: Mechanistic interpretability shows that when a model undergoes Supervised Fine-Tuning or shifting domain distribution, it biases its internal representations (Function Vectors) toward the new unstructured task. This causes the model to actively derail or overwrite the precise structural circuits (like Induction Heads) required for in-context learning. Stage 58/H026 showed exactly this: a model trained only on simple data scored 0.194 on a copy probe, but dropped to 0.061 (chance) after fine-tuning on broad text8 data. The structural circuit was destroyed despite NLL improvements.
...
- **Scale Breakdown in Proxy Data Mixing**: While Data Mixing Laws theoretically predict target mixtures, the assumption of scale invariance breaks down across large capacity gaps. Small proxies suffer intense "capacity competition" and heavily penalize complex domains they lack the representation space to learn, whereas larger targets can isolate them. Stage 59 proved this empirically: a 3.2M proxy over-predicted an 85M model's mixing toll by 2.09x, proving offline proxies are structurally broken for scaling mixture laws at this extreme low end.
